hook.Add("HUDPaint", "AFKT", function()
    if LocalPlayer():GetNWBool("LeadBot_AFK") then
        ...
        draw.SimpleTextOutlined("You are currently AFK", "ScoreboardDefaultTitle", ScrW() / 2, ScrH() / 1.7, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1,  Color(0, 0, 0,255))
        draw.SimpleTextOutlined("Jump to get out of AFK", "ScoreboardDefaultTitle", ScrW() / 2, ScrH() / 1.6, Color(255, 255, 255, aa), TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, Color(0, 0, 0, aa))
    end
end)
...
hook.Add("CreateMove", "LeadBot_AFK", function(cmd)
    if LocalPlayer():GetNWBool("LeadBot_AFK") and cmd:KeyDown(IN_JUMP) or cmd:KeyDown(IN_FORWARD) then
